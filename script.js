// ======================================================
//  EIRA BAG STORE — script.js (Responsive Update)
// ======================================================
...
function showSuccess() {
  ...
  // *** STEP 1: Kurangi stok DULU sebelum cart dikosongkan ***
  cart.forEach(item => {
    const p = products.find(x => x.id === item.id);
    if (p) {
      p.stock = Math.max(0, p.stock - item.qty);
      console.log(`Stok ${p.name} sekarang: ${p.stock}`); // debug
    }
  });

  // *** STEP 2: Baru kosongkan cart ***
  cart = [];
  updateBadge();
  resetBuyerForm();
  selKurir = null; selKurirCost = 0; selKurirName = '';
